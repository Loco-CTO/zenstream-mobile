...
class DetailScreensTest {
    ...
    @Test
    fun episodeOverlayUsesSeriesTitleAndOpensSeries() {
        val series = MediaItem("series", "Example Series", type = "Series")
        ...
        composeRule.onNodeWithTag("detail_overlay_title").assertIsDisplayed()
        composeRule
    ...
    fun catalogDetailOverlayKeepsBackVisibleAndRevealsTitleAfterScrolling() {
        ...
        composeRule.setContent {
            ZenStreamTheme {
                ...
                ObserveDetailScroll(listState) {
                    scrolled = it
                    if (it) scrolledAtLeastOnce = true
                }
                Box(Modifier.fillMaxSize()) {
                    LazyColumn(
                        modifier = Modifier.fillMaxSize().testTag("overlay_scroll_content"),
                        state = listState,
                    ) {
                        repeat(40) {
    ...
    @Test
    fun detailContentStartsAtRootWithoutTopBarGap() {
        ...
        composeRule.onNodeWithTag("detail_content_list").assertTopPositionInRootIsEqualTo(0.dp)
    }
